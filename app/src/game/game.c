...
void create_game(int argc, char** argv) {
	...
	memset(g.bg_textures, 0, sizeof(g.bg_textures));
	...
	g.renderer = renderer_create(&g, g.icontext, g.window, sprite_sheet, MAX_BP_RENDER, MAX_FOOD_RENDER, 128,
	                             font_sheet, g.bg_tex, 64);
	backgrounds_init(&g);
	...
	while (!ig_window_closed(g.window) && !g.game_quit) {
		...
		input_data.t_pressed = ig_keyboard_key_pressed(g.keyboard, GLFW_KEY_T);
		input_data.u_pressed = ig_keyboard_key_pressed(g.keyboard, GLFW_KEY_U);
		input_data.seven_pressed = ig_keyboard_key_pressed(g.keyboard, GLFW_KEY_7);
		if (input_data.u_pressed) backgrounds_cycle(&g);
		if (input_data.seven_pressed) {
			g.settings_instance.skinless = !g.settings_instance.skinless;
		...
		if (g.connected) {
			...
			if (g.config.bot && !g.snake_null) {
				float bot_now = input_data.ctm;
				if (g.config.last_bot_mtm <= 0.0f ||
				    bot_now - g.config.last_bot_mtm >= 100.0f) {
					g.config.last_bot_mtm = bot_now;
					sbot_go(&g);
				}
			}
