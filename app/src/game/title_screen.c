...
void title_screen(game* g) {
	...
	if (g->respawn) {
		...
		if (g->setting_skin) {
			...
			ImVec2 credit_size;
			igCalcTextSize(&credit_size, "By Stevin  |  Ignite engine", NULL, false, 0);
			igSetNextWindowPos((ImVec2) { 6, g->icontext->default_frame.resolution.y - credit_size.y - 6 }, ImGuiCond_None, (ImVec2) { 0, 0 });
			igSetNextWindowSize((ImVec2) { credit_size.x + 8, credit_size.y + 4 }, ImGuiCond_None);
			igBegin("credits_win", NULL, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoInputs);
			igSetCursorPosX(0);
			igSetCursorPosY(0);
			igTextColored((ImVec4) { 1.0f, 1.0f, 1.0f, 0.35f }, "By Stevin  |  Ignite engine");
			igEnd();
			igPopFont();
		}
	}
