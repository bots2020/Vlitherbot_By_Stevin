...
void oef(game* g, struct mg_connection* c, const input_data* input_data) {
	...
	if (!g->snake_null) {
		...
		} else if (input_data->t_pressed) {
			g->config.bot = !g->config.bot;
			if (g->config.bot) {
				sbot_init(g);
				g->config.last_bot_mtm = 0.0f;
			}
		...
		g->config.wmd = input_data->btn_down ||
		                 (g->config.bot && g->bot.output.accel);
		...
		if (want_e && input_data->ctm - g->config.last_e_mtm > 50) {
			...
			if (!g->config.bot) {
				xm = input_data->mouse_pos.x - g->icontext->default_frame.resolution.x / 2;
				ym = input_data->mouse_pos.y - g->icontext->default_frame.resolution.y / 2;
			}
