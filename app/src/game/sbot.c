...
static inline float sbot_map_radius(game* g) {
  return (float)g->config.grd * 0.98f;
}
...
#define BOT_CIRCLE_EXIT_MARGIN 400
#define BOT_FOOD_SCAN_RANGE_MULT 55.0f
#define BOT_FOOD_SCAN_MAX 1200
...
static inline int ang_index(float angle) {
  float a = angle < 0 ? angle + X2PI : angle;
...
static void get_collision_points(game* g) {
  B.coll_pts_n = 0;
  memset(B.coll_angles_set, 0, sizeof(B.coll_angles_set));

  float far_d2 = (B.width * 50.0f) * (B.width * 50.0f);
  int ns = ig_darray_length(g->os.snakes);

  for (int i = 0; i < ns; i++) {
    snake* s = g->os.snakes + i;
    ...
    int pn = ig_darray_length(s->pts);
    int step = pn > 128 ? 4 : (pn > 64 ? 2 : 1);
  ...
  float view_ang = atan2f(g->config.view_yy - (float)g->config.grd,
                          g->config.view_xx - (float)g->config.grd);
  float dist_to_ctr = sqrtf(dist2(B.x, B.y, (float)g->config.grd, (float)g->config.grd));
  bool near_wall = (sbot_map_radius(g) - dist_to_ctr) < 1000.0f;

  if (near_wall) {
    float bpw = BORDER_PT_RADIUS * 2.0f;
    float brad = sbot_map_radius(g) + BORDER_PT_RADIUS;
    for (int k = -3; k <= 3; k++) {
      float wa = view_ang + (k * bpw) / sbot_map_radius(g);
      float wx = (float)g->config.grd + brad * cosf(wa);
      float wy = (float)g->config.grd + brad * sinf(wa);
...
static bool check_collision(game* g, bool scan) {
  if (scan) get_collision_points(g);
  ...
  for (int i = 0; i < B.coll_pts_n; i++) {
    ...
    if (circle_intersect(B.head_circle, col, &ip) && in_front(ip.x, ip.y)) {
      if (cp->type == 0) {
        snake* s = g->os.snakes + cp->si;
        g->bot.output.accel = s->sp > 10.0f;
      } else {
        g->bot.output.accel = (bool)B.default_accel;
      }
      avoid_point(&ip);
      return true;
    }
...
static bool check_encircle(game* g) {
  ...
  if (high > (int)(MAXARC * ENCIRCLE_THRESH)) {
    ...
    if (high != MAXARC && high_si < ns)
      g->bot.output.accel = g->os.snakes[high_si].sp > 10.0f;
    else
      g->bot.output.accel = (bool)B.default_accel;
    return true;
  }
...
static void populate_pts(game* g) {
  B.bpts_n = 0;
  B.bpts_len = 0.0f;
  int ns = ig_darray_length(g->os.snakes);
  snake* me = NULL;
  for (int i = 0; i < ns; i++)
    if (g->os.snakes[i].id == B.id) {
      me = g->os.snakes + i;
      break;
    }
  if (!me) return;
  ...
  int pn = ig_darray_length(me->pts);
...
static void follow_circle_self(game* g) {
  populate_pts(g);
  ...
  for (int i = 0; i < ns; i++) {
    ...
    int pn = ig_darray_length(sk->pts);
    int pstep = pn > 48 ? 2 : 1;
    for (int j = 0; j < pn; j += pstep) {
  ...
  if (near_wall) {
    ...
    for (int k = -2; k <= 2; k++) {
      float wa = view_ang + (k * bpw) / sbot_map_radius(g);
      v2 wp = {(float)g->config.grd + brad * cosf(wa),
               (float)g->config.grd + brad * sinf(wa)};
...
static void to_circle(game* g) {
  ...
  int pn = ig_darray_length(me->pts), checked = 0;
...
static inline bool food_behind_body(float fx, float fy) {
  float dx = fx - B.x, dy = fy - B.y;
  if (dx * B.ca + dy * B.sa < 0.0f) return true;
  return fabsf(-dx * B.sa + dy * B.ca) < B.width;
}

static void compute_food_goal(game* g) {
  memset(B.food_angles_set, 0, sizeof(B.food_angles_set));
  float food_far_d2 =
      (B.width * BOT_FOOD_SCAN_RANGE_MULT) * (B.width * BOT_FOOD_SCAN_RANGE_MULT);
  int fn = ig_darray_length(g->foods);
  int scanned = 0;
  for (int i = fn - 1; i >= 0; i--) {
    if (scanned >= BOT_FOOD_SCAN_MAX) break;
    food* fo = g->foods + i;
    if (fo->eaten) continue;
    scanned++;
    float fd2 = dist2(B.x, B.y, fo->xx, fo->yy);
    if (fd2 > food_far_d2) continue;
    if (food_behind_body(fo->xx, fo->yy)) continue;
...
static void delay_action(game* g) {
  ...
  bool playing = false;
  for (int i = 0; i < ns; i++)
    if (g->os.snakes[i].id == B.id && !g->os.snakes[i].dead) {
      playing = true;
      break;
    }
  ...
  if (playing) {
    if (B.stage == 0) {
      ...
      B.goal = B.has_food ? (v2){B.current_food.x, B.current_food.y}
                          : (v2){(float)g->config.grd, (float)g->config.grd};
    } else if (B.stage == 1) {
      to_circle(g);
    }
  }
...
static int snake_score(game* g, snake* sk) {
  if (sk->sct < 0 || sk->fam < 0) return 0;
  int sct = sk->sct;
  int n = ig_darray_length(g->config.fpsls);
  if (n <= 0) return 0;
  if (sct >= n) sct = n - 1;
  return (int)floorf(
      (g->config.fpsls[sct] + sk->fam / g->config.fmlts[sct] - 1.0f) * 15.0f -
      5.0f);
}

static void every(game* g) {
  int ns = ig_darray_length(g->os.snakes);
  if (ns == 0) return;
  snake* me = g->os.snakes;
  B.id = me->id;
...
void sbot_init(game* g) {
  ...
  B.circle_locked = false;
  g->bot.output.accel = false;
  g->bot.output.xm = 0;
  g->bot.output.ym = 0;
}

static void sbot_apply_output(game* g) {
  g->bot.output.xm = (B.goal.x - g->config.view_xx) * g->config.gsc;
  g->bot.output.ym = (B.goal.y - g->config.view_yy) * g->config.gsc;
}

static bool sbot_should_circle(void) {
  if (!B.enable_follow_circle) return false;
  int enter = (int)B.follow_circle_length;
  int exit_at = enter - BOT_CIRCLE_EXIT_MARGIN;
  if (exit_at < 0) exit_at = 0;
  if (B.snake_len >= enter) B.circle_locked = true;
  else if (B.snake_len < exit_at) B.circle_locked = false;
  return B.circle_locked;
}

void sbot_go(game* g) {
  sbot* bot = &g->bot;
  B.radius_mult = (float)g->settings_instance.bot_radius_mult;
  B.follow_circle_length = (float)g->settings_instance.bot_follow_circle_score;

  if (ig_darray_length(g->os.snakes) == 0) return;

  every(g);

  bool circle_mode = sbot_should_circle();
  if (!circle_mode) {
    B.stage = 0;
  } else if (B.stage < 2) {
    to_circle(g);
    if (B.stage < 2) {
      populate_pts(g);
      if (B.bpts_len >= 9.0f * B.width) B.stage = 2;
    }
  }
  ...
  if (B.stage == 2) {
    ...
    follow_circle_self(g);
  } else if (check_collision(g, true) ||
             (B.enable_encircle && check_encircle(g))) {
  ...
  if (B.stage == 0) delay_action(g);

  sbot_apply_output(g);
}

void sbot_destroy(game* g) { (void)g; }