...
void settings_screen(game* g) {
	...
	igSetNextItemWidth(igGetColumnWidth(1) - style->WindowPadding.x - 5);
	if (igBeginCombo("##background", backgrounds_label(g->settings_instance.bg_index), ImGuiComboFlags_None)) {
		for (int bi = 0; bi < BG_COUNT; bi++) {
			if (igSelectable_Bool(backgrounds_label(bi), g->settings_instance.bg_index == bi,
			                      ImGuiSelectableFlags_None, (ImVec2) { 0, 0 })) {
				backgrounds_apply(g, bi);
			}
		}
		igEndCombo();
	}
	igSetNextItemWidth(igGetColumnWidth(1) - style->WindowPadding.x - 5);
	if (igBeginCombo("##snake_style", g->settings_instance.skinless ? "Skinless" : "Normal", ImGuiComboFlags_None)) {
		if (igSelectable_Bool("Normal", !g->settings_instance.skinless, ImGuiSelectableFlags_None, (ImVec2) { 0, 0 })) {
			g->settings_instance.skinless = false;
			apply_render_quality(g);
		}
		if (igSelectable_Bool("Skinless", g->settings_instance.skinless, ImGuiSelectableFlags_None, (ImVec2) { 0, 0 })) {
			g->settings_instance.skinless = true;
			apply_render_quality(g);
		}
		igEndCombo();
	}
	...
	igBeginDisabled(!g->settings_instance.skinless);
	igCheckbox("##center_line", &g->settings_instance.snake_center_line);
	igEndDisabled();
	...
	igColumns(2, NULL, false);
	igAlignTextToFramePadding();
	igText("Circle after score");
	igAlignTextToFramePadding();
	igText("Radius mult");
	igNextColumn();
	igSetNextItemWidth(igGetColumnWidth(1) - style->WindowPadding.x - 5);
	igSliderInt("##bot_circle", &g->settings_instance.bot_follow_circle_score, 1000, 6000, "%d",
		ImGuiSliderFlags_AlwaysClamp);
	igSetNextItemWidth(igGetColumnWidth(1) - style->WindowPadding.x - 5);
	igSliderInt("##bot_rad", &g->settings_instance.bot_radius_mult, 10, 40, "%dx",
		ImGuiSliderFlags_AlwaysClamp);
	igColumns(1, NULL, false);
	...
	HOTKEY_INFO("(T)", "Toggle bot");
	HOTKEY_INFO("(U)", "Cycle background");
	HOTKEY_INFO("(7)", "Toggle snake style");
