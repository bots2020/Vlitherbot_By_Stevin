...
void redraw(game* g, const input_data* input_data) {
	...
	renderer_push_bg(g->renderer, &(sprite_instance) {
		...
		.color = {
			.x = backgrounds_is_black(g->settings_instance.bg_index) ? 0.0f : 1.0f,
			.y = backgrounds_is_black(g->settings_instance.bg_index) ? 0.0f : 1.0f,
			.z = backgrounds_is_black(g->settings_instance.bg_index) ? 0.0f : 1.0f,
			.w = 1.0f }
	});
	...
	for (int i = 0; i < snakes_len; i++) {
		...
		if (o->iiv) {
			...
			bool skinless = g->settings_instance.skinless;
			bool is_me = !g->snake_null && o == g->os.snakes;
			int base_cv = (o->skin_data_len > 0) ? o->skin_data[0] : 0;
			ig_vec3* base_col = g->config.color_groups + base_cv;
			/* skinless: qsm baixo = gomos densos = corpo liso/contínuo */
			float qsm_place = skinless ? 0.32f : g->config.qsm;
			float sep = o->sep * qsm_place;
			...
			if (o->tsp > o->fsp && g->config.show_boost) {
				...
				if (skinless) glsz *= 0.88f;
				float dj = 4;
				float boost_mul = skinless ? 0.21f : 0.38f;
				int boost_step = (skinless && bp > 96) ? 2 : 1;

				for (int j = bp - 1; j >= 0; j -= boost_step) {
					if (g->config.pbu[j] != 2) continue;
					px = (mww2 + ((g->config.pbx[j] - g->config.view_xx) * g->config.gsc));
					py = (mhh2 + ((g->config.pby[j] - g->config.view_yy) * g->config.gsc));
					float a = v * mr * boost_mul * (.6f + .4f * cosf(j / dj - 1.15f * o->sfr));
					if (skinless) {
						renderer_push_bp(g->renderer, &(bp_instance) {
							.circ = { .x = px - glsz, .y = py - glsz, .z = 0, .w = glsz * 2 },
							.ratios = { .x = 0, .y = 1 },
							.color = { .x = base_col->x, .y = base_col->y, .z = base_col->z, .w = a },
							.shadow = 1,
							.flat = 0,
							.center_line = 0
						});
			...
			if (skinless) {
				float body_d = g->config.gsc * 2.0f * lsz;
				int body_step = (!is_me && bp > 128) ? 2 : 1;
				for (int j = bp - 1; j >= 0; j -= body_step) {
					if (g->config.pbu[j] < 1) continue;
					px = mww2 + ((g->config.pbx[j] - g->config.view_xx) * g->config.gsc);
					py = mhh2 + ((g->config.pby[j] - g->config.view_yy) * g->config.gsc);
					renderer_push_bp(g->renderer, &(bp_instance) {
						.circ = { .x = px - g->config.gsc * lsz, .y = py - g->config.gsc * lsz, .z = 1 - 0, .w = body_d },
						.ratios = { .x = sinf(g->config.pba[j]), .y = cosf(g->config.pba[j]) },
						.color = { .x = base_col->x, .y = base_col->y, .z = base_col->z, .w = v },
						.shadow = 0,
						.eye = 0,
						.flat = 1,
						.center_line = 0
				...
				if (is_me && g->settings_instance.snake_center_line && bp > 2) {
					float line_start_r2 = (7.0f * ssc) * (7.0f * ssc);
					int line_from = 0;
					for (int j = 0; j < bp; j++) {
						if (g->config.pbu[j] != 2) continue;
						float dx = g->config.pbx[j] - hx;
						float dy = g->config.pby[j] - hy;
						if (dx * dx + dy * dy >= line_start_r2) {
							line_from = j;
							break;
						}
					}
					if (line_from < bp - 1) {
						ImU32 line_col = igColorConvertFloat4ToU32((ImVec4) { 1, 1, 1, 0.75f });
						float prev_sx = 0, prev_sy = 0;
						bool has_prev = false;
						for (int j = line_from; j < bp; j++) {
							if (g->config.pbu[j] != 2) continue;
							float sx = mww2 + (g->config.pbx[j] - g->config.view_xx) * g->config.gsc;
							float sy = mhh2 + (g->config.pby[j] - g->config.view_yy) * g->config.gsc;
							if (has_prev)
								ImDrawList_AddLine(draw_list, (ImVec2) { prev_sx, prev_sy },
								                   (ImVec2) { sx, sy }, line_col, 1.25f);
							prev_sx = sx;
							prev_sy = sy;
							has_prev = true;
						}
				...
				for (int j = bp - 1; j >= 0; j--) {
					if (g->config.pbu[j] >= 1) {
						if (g->config.shadow && j >= 1 && g->config.pbu[j - 1] == 2) {
