...
void hud(game* g, const input_data* input_data) {
	...
	if (g->config.bot && !g->snake_null) {
		ImVec2 bot_sz;
		igPushFont(g->renderer->fonts[RENDERER_FONT_SMALL]);
		igCalcTextSize(&bot_sz, "BOT ON", NULL, false, 0);
		float bot_y = g->icontext->default_frame.resolution.y - fps_text_size.y * 4;
		if (g->settings_instance.show_fps) bot_y -= fps_text_size.y + 2.0f;
		ImVec2 bot_pos;
		igSetCursorPosX(4);
		igSetCursorPosY(bot_y);
		igGetCursorScreenPos(&bot_pos);
		ImU32 glow_col = igColorConvertFloat4ToU32((ImVec4) { 0.2f, 0.95f, 0.4f, 0.4f });
		ImU32 text_col = igColorConvertFloat4ToU32((ImVec4) { 0.35f, 1.0f, 0.55f, 0.95f });
		ImDrawList_AddText_Vec2(draw_list, (ImVec2) { bot_pos.x + 1.0f, bot_pos.y }, glow_col, "BOT ON", NULL);
		ImDrawList_AddText_Vec2(draw_list, (ImVec2) { bot_pos.x - 1.0f, bot_pos.y }, glow_col, "BOT ON", NULL);
		ImDrawList_AddText_Vec2(draw_list, (ImVec2) { bot_pos.x, bot_pos.y + 1.0f }, glow_col, "BOT ON", NULL);
		ImDrawList_AddText_Vec2(draw_list, bot_pos, text_col, "BOT ON", NULL);
		igPopFont();
	}
	...
	static int mem_warn_cd;
	static int mem_bar_tick;
	mem_bar_tick++;
	if (mem_bar_tick >= 90) {
		mem_bar_tick = 0;
		igSetCursorPosX(4);
		igSetCursorPosY(4);
		ImVec4 bar_col = {};
		igImLerp_Vec4(&bar_col, (ImVec4) { 0, 1, 0, 0.5f }, (ImVec4) { 1, 0, 0, 0.5f }, bp_mem);
	...
	if ((bp_mem >= 0.8f || food_mem >= 0.8f) && mem_warn_cd <= 0) {
		mem_warn_cd = 180;
		message msg = {
