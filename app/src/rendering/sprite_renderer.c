...
void sprite_renderer_set_texture(sprite_renderer* sprite_renderer,
                                 const ig_texture* texture,
                                 ig_context* context) {
  if (!sprite_renderer || !texture) return;
  vkUpdateDescriptorSets(context->device, 1, &(VkWriteDescriptorSet) {
    .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
    .pNext = NULL,
    .dstSet = sprite_renderer->sprite_sheet_ptr,
    .dstBinding = 0,
    .dstArrayElement = 0,
    .descriptorCount = 1,
    .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
