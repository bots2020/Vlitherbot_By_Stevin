...
void apply_render_quality(game* g) {
...
void load_settings(game* g) {
	...
	size_t legacy_size = offsetof(settings, skinless);
	if (file_size >= (long) sizeof(settings)) {
		fread(&g->settings_instance, sizeof(settings), 1, file);
	} else if (file_size >= (long) legacy_size) {
		memset(((char*) &g->settings_instance) + legacy_size, 0,
		       sizeof(settings) - legacy_size);
		fread(&g->settings_instance, legacy_size, 1, file);
		g->settings_instance.skinless = false;
		g->settings_instance.snake_center_line = true;
	} else {
		fclose(file);
		return;
	}
	...
	if (g->settings_instance.bg_index < 0 || g->settings_instance.bg_index >= BG_COUNT)
		g->settings_instance.bg_index = 0;
