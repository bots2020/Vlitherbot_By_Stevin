...
layout (location = 4) in float in_bp_flat;
layout (location = 5) in float in_center_line;

void main() {
	float l = length(out_uv * 2.0 - 1.0);
	float f = (1.0 - l);

	if (in_bp_flat > 0.5 && in_eye < 0.5) {
		/* borda quase dura; leve fusão nos gomos sobrepostos */
		float a = smoothstep(0.0, 0.018, f);
		if (a == 0.0) discard;
		color = vec4(out_color.rgb, out_color.a * a);
		return;
	}
